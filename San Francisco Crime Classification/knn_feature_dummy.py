...
def data_processing(train,test):
    ...
    aaa=train.shape[0]
    bbb=test.shape[0]
    print(train[features].sum()/aaa)
    print(test[features].sum()/bbb)
    ...
    print("feature_len",len(features))
...
def SVM_model(train,test,features):
    ...
    ans = clf.predict_proba(test[features].values)
    ...
    csvfile = 'results/knn-submit.csv'
